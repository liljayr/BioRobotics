...
class CMAC:
    def __init__(self, n_rfs, xmin, xmax, D, beta):
        """ Initialize the basis function parameters and output weights """
        self.n_rfs = n_rfs

        self.mu = np.zeros((D, self.n_rfs))
        self.sigma = np.zeros(D)
        crossval = 0.8 # has to be between 0 and 1 !

        for k in range(D): # k = layer
    ...
    def predict(self, x):
        ...
        """
        phi = np.zeros((self.D, self.n_rfs))
...
if __name__ == '__main__':
    ...
    c = CMAC(n_rfs, xmin, xmax, D, 1e-2)
    print(c.w.shape)

    for _ in range(1000):
        e_vec = []
        for x1 in np.linspace(0, 1, n_rfs):
            for x2 in np.linspace(0, 1, n_rfs):
                for x3 in np.linspace(0, 1, n_rfs):
                    x = [x1, x2, x3]
