...
class SimulationFunctions:
    ...
    ## Converting wrist positions into target joint angels 
    ...
    ## Compute desired torque for target joint angels
    def pdcontroller(self,desired_angle, delayed_angle, delayed_velocity, noise_coefficient): 
        kd = 5.0 #damping parameter of pd controller
        kp = 50.0 #gain parameter of pd controller
        ...
        noise = noise_coefficient * np.random.randn(2) #We get 2 random values each time we run through the for-loop in "exercise2.py" 
        noisy_torque = noise + torque 
    ...
    ## With torques as input, plant control the arm
    ...
    ## With torque as input, fkinematics finds resulting wrist position
