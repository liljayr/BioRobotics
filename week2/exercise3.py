...
#Script somulates control of single joint arm using feedback and feedforward control
#On each time step, the forward model of arm is used to estimate the:
# - angle
# - velocity
# - acceleration of the joint

#The is used to compute a forward model torque which is combined with a feedback-based torque (computed w. a delayed copy of the actual joint)

#The actual predicted, delayed and target trajectory of the joint angle, the perturbation and total torque applied
# are plotted over time steps of the simulation.
...
# Sensory delay (s), is added as incements (for-loop) in simulation
delay =  0 
#delay =  0.125

# ### 2.3.1
# for delay in np.arange(0,0.125,0.025):
...
pert_amp = 1      #Perturbation turned on 
#pert_amp = 0     #Perturbation turned off

# Gain and damping parameters for forward model
Kp_forward = 5      #Forward model turned on
#Kp_forward = 0     #Forward model turned off
Kd_forward = 0.3    #Forward model turned on
#Kd_forward = 0     #Forward model turned off

# Gain and damping parameters for feedback model
#Kp_feedback = 5    #Feedback model turned on
Kp_feedback = 0     #Feedback model turned off
#Kd_feedback = 0.3  #Feedback model turned on
Kd_feedback = 0     #Feedback model turned off
...
plt.title("Forward on, Feedback off, pert_amp = 1, delay = 0 ")
plt.show()